...
test('Model console styling preserves responsive commercial layout', () => {
  ...
  assert.match(cssBlock('.models-primary-select'), /font-family:\s*var\(--font-mono\)/)
...
test('Model console locale keys are present', () => {
  for (const key of ['manageFallbacks', 'testPrimary', 'locateModel', 'addFallback', 'removeFallback', 'invalidRefs']) {
