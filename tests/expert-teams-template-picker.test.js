...
test('group template picker is an accessible, dismissible dialog', () => {
  ...
  assert.match(body, /role="dialog"/, 'picker should be a dialog')
  assert.match(body, /aria-modal="true"/, 'picker should be modal')
  assert.match(body, /aria-labelledby=/, 'picker should be labelled')
  assert.match(body, /e\.key === 'Escape'/, 'Escape must close the picker')
  assert.match(body, /previousFocus/, 'focus should be restored on close')
  assert.match(body, /removeEventListener\('keydown'/, 'keydown listener must be cleaned up on close')
})
