...
export default {
  ...
  gatewayTitle: _('Gateway 管理', 'Gateway Management', 'Gateway 管理', 'Gateway 管理', 'Gateway 관리', 'Quản lý Gateway', 'Gestión de Gateway', 'Gestão do Gateway', 'Управление Gateway', 'Gestion de Gateway', 'Gateway-Verwaltung'),
  gatewayDesc: _('查看 Gateway 状态、处理端口归属冲突、启动或停止当前绑定实例', 'Check Gateway status, resolve port ownership conflicts, and start or stop the bound instance', '查看 Gateway 狀態、處理連接埠歸屬衝突、啟動或停止目前綁定實例', 'Gateway の状態確認、ポート所有権の競合処理、現在バインドされたインスタンスの起動・停止', 'Gateway 상태 확인, 포트 소유권 충돌 처리, 현재 바인딩된 인스턴스 시작 또는 중지', 'Kiểm tra trạng thái Gateway, xử lý xung đột quyền sở hữu cổng và khởi động hoặc dừng phiên bản đã liên kết', 'Revisa el estado de Gateway, resuelve conflictos de propiedad de puertos e inicia o detén la instancia vinculada', 'Verifique o status do Gateway, resolva conflitos de propriedade de porta e inicie ou pare a instância vinculada', 'Проверка статуса Gateway, устранение конфликтов владения портом и запуск или остановка привязанного экземпляра', 'Vérifier l’état de Gateway, résoudre les conflits de propriété de port et démarrer ou arrêter l’instance liée', 'Gateway-Status prüfen, Port-Zuordnungskonflikte lösen und die gebundene Instanz starten oder stoppen'),
