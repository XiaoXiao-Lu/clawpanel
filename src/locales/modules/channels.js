...
export default {
  ...
  groupAllowFromPh: _('可选，逗号分隔群组或会话 ID', 'Optional, comma-separated group or thread IDs'),
  proxyHint: _('HTTP/HTTPS/SOCKS5 代理地址，如 http://127.0.0.1:7890', 'HTTP/HTTPS/SOCKS5 proxy address, e.g. http://127.0.0.1:7890'),
  ...
  removeStillConfigured: _('删除后仍检测到该配置，请重试或重启应用', 'The configuration is still present after deletion. Retry or restart the app.', '刪除後仍偵測到該設定，請重試或重新啟動應用程式'),
  ...
  accountIdPlaceholder: _('留空为默认账号；创建后不可修改', 'Leave empty for the default account; identifiers cannot be changed later', '留空為預設账號；建立後不可修改'),
  accountIdHint: _('每个账号对应一个独立机器人；账号标识创建后不可修改。', 'Each account is an independent bot; its identifier cannot be changed after creation.', '每個账號對應一個獨立機器人；账號標識建立後不可修改。'),
  accountIdRequiredHint: _('请输入一个标识名（如 bot2、work-bot），用于区分不同账号。', 'Enter a unique identifier (e.g. bot2, work-bot) to distinguish multiple accounts.', '請輸入一個標識名（如 bot2、work-bot），用於區分不同账號。'),
  accountIdRequired: _('请填写账号标识', 'Account identifier is required', '請填寫账號標識'),
  accountIdReserved: _('default 是系统保留账号名，请换一个账号标识', 'default is reserved; choose another account identifier', 'default 是系統保留帳號名稱，請換一個帳號識別碼'),
  ...
  // 运行态面板
  runtimeRefresh: _('刷新状态', 'Refresh Status', '重新整理狀態'),
  runtimeRefreshShort: _('刷新', 'Refresh', '重新整理'),
  runtimeRefreshTitle: _('刷新 Gateway 渠道状态', 'Refresh Gateway channel status', '重新整理 Gateway 頻道狀態'),
  runtimeStart: _('启动', 'Start', '啟動'),
  runtimeStartTitle: _('启动该渠道运行时', 'Start this channel runtime', '啟動該頻道執行時'),
  runtimeStop: _('停止', 'Stop', '停止'),
  runtimeStopTitle: _('停止该渠道运行时', 'Stop this channel runtime', '停止該頻道執行時'),
  runtimeLogout: _('注销', 'Logout', '登出'),
  runtimeLogoutTitle: _('注销该渠道账号登录态', 'Logout this channel account', '登出該頻道帳號登入態'),
  runtimeRefreshing: _('刷新中', 'Refreshing...', '重新整理中'),
  runtimeProcessing: _('处理中', 'Processing...', '處理中'),
  runtimeRefreshed: _('渠道运行态已刷新', 'Channel runtime status refreshed', '頻道運行態已重新整理'),
  runtimeGatewayNotReady: _('Gateway WebSocket 未连接，请先启动 OpenClaw Gateway', 'Gateway WebSocket not connected. Please start OpenClaw Gateway first.', 'Gateway WebSocket 未連線，請先啟動 OpenClaw Gateway'),
  runtimeUnsupported: _('当前 OpenClaw 内核不支持通用渠道运行态，请升级到新版内核；配置编辑仍可继续使用。', 'Current OpenClaw kernel does not support runtime status. Please upgrade to a newer version; config editing continues to work.', '目前 OpenClaw 核心不支援通用頻道運行態，請升級到新版核心；設定編輯仍可繼續使用。'),
}
