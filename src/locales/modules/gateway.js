...
export default {
  ...
  // Session 配置
  sessionTitle: _('会话记忆', 'Session Memory', '對話記憶'),
  sessionDesc: _('控制不同渠道间的消息记忆和身份关联', 'Control cross-channel memory and identity linking', '控制跨渠道記憶和身份關聯'),
  dmScopeLabel: _('私信会话隔离', 'DM Session Isolation', '私訊對話隔離'),
  dmScopeHint: _('决定不同人的私信是否分开记忆。多人使用时建议开启隔离', 'Determines whether DMs from different people are isolated. Recommended for multi-user setups', '決定不同人的私訊是否分開記憶。多人使用時建議開啟隔離'),
  dmScopeMain: _('共享会话', 'Shared', '共享對話'),
  dmScopeMainDesc: _('所有人私信共享同一会话（默认，单用户适用）', 'All DMs share one session (default, for single user)', '所有人私訊共享同一對話（預設，單用戶適用）'),
  dmScopePerPeer: _('按用户隔离', 'Per User', '按用戶隔離'),
  dmScopePerPeerDesc: _('同一用户跨渠道共享，不同用户分开', 'Same user shares across channels, different users isolated', '同一用戶跨渠道共享，不同用戶分開'),
  dmScopePerChannelPeer: _('按渠道+用户', 'Per Channel+User', '按渠道+用戶'),
  dmScopePerChannelPeerDesc: _('每个渠道的每个用户独立（推荐多用户）', 'Each channel+user is isolated (recommended for multi-user)', '每個渠道的每個用戶獨立（推薦多用戶）'),
  dmScopePerAccountChannelPeer: _('按账号+渠道+用户', 'Per Account+Channel+User', '按帳號+渠道+用戶'),
  dmScopePerAccountChannelPeerDesc: _('多账号场景的最细粒度隔离', 'Finest granularity for multi-account setups', '多帳號場景的最細粒度隔離'),
  identityLinksTitle: _('身份关联', 'Identity Links', '身份關聯'),
  identityLinksDesc: _('将不同渠道的同一个人关联起来，让他们跨渠道共享记忆。先设置上面的隔离级别为「按用户」或「按渠道+用户」才有效', 'Link the same person across channels to share memory. Requires dmScope set to per-peer or per-channel-peer', '將不同渠道的同一個人關聯起來，讓他們跨渠道共享記憶。需先設定上面的隔離級別為「按用戶」或「按渠道+用戶」才有效'),
  identityNamePlaceholder: _('身份名称，如 alice', 'Identity name, e.g. alice', '身份名稱，如 alice'),
  peerIdPlaceholder: _('渠道:用户ID，如 telegram:123456', 'channel:userId, e.g. telegram:123456', '渠道:用戶ID，如 telegram:123456'),
  addIdentity: _('添加身份', 'Add Identity', '添加身份'),
  addPeerId: _('添加渠道ID', 'Add Peer ID', '添加渠道ID'),
  removeIdentity: _('删除', 'Remove', '刪除'),
  identityLinksEmpty: _('暂无身份关联，点击「添加身份」开始', 'No identity links yet. Click "Add Identity" to start', '暫無身份關聯，點擊「添加身份」開始'),
  identityLinksHint: _('💡 提示：先在上方选择隔离级别为「按用户」或「按渠道+用户」，然后在此处添加身份。每个身份可以绑定多个渠道账号，OpenClaw 会自动识别同一人跨渠道的消息', 'Tip: Set isolation level above to per-user or per-channel-user, then add identities here. Each identity can link multiple channel accounts so OpenClaw recognizes the same person across channels', '提示：先在上方選擇隔離級別為「按用戶」或「按渠道+用戶」，然後在此處添加身份。每個身份可綁定多個渠道帳號'),
  selectChannel: _('选择渠道', 'Select Channel', '選擇渠道'),
  peerIdHintDefault: _('格式：渠道:用户ID，如 telegram:123456。保存后会自动组合', 'Format: channel:userId, e.g. telegram:123456. Auto-combined on save.', '格式：渠道:用戶ID，如 telegram:123456。儲存後自動組合'),
}
