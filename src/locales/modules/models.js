...
export default {
  ...
  undoN: _('↩ 撤销 ({n})', '↩ Undo ({n})'),
  undone: _('已撤销', 'Undone'),
  providerHint: _('先选当前默认模型，再按需要设置备用模型顺序。服务商只是模型来源；模型切换、连通测试和兜底链路都在下方统一管理，修改后会自动保存。', 'Choose the current default model first, then arrange fallback priority as needed. Providers are just model sources; switching, tests, and fallback routing are managed below and auto-saved.', '先選目前預設模型，再依需要設定備用模型順序。服務商只是模型來源；模型切換、連通測試和備援鏈路都在下方統一管理，修改後會自動儲存。'),
  ...
  allProviders: _('全部', 'All', '全部'),
  colModel: _('模型名称', 'Model', '模型名稱'),
  colProvider: _('服务商', 'Provider', '服務商'),
  colStatus: _('状态', 'Status', '狀態'),
  colLatency: _('延迟', 'Latency', '延遲'),
  colActions: _('操作', 'Actions', '操作'),
  normal: _('正常', 'Normal', '正常'),
  ...
  totalModels: _('模型总数', 'Total models', '模型總數'),
  totalProviders: _('服务商', 'Providers', '服務商'),
  fallbackShort: _('备选', 'Fallback', '備選'),
  invalidRefs: _('失效引用', 'Invalid refs', '失效引用'),
  notTested: _('未测试', 'Not tested', '未測試'),
  addFallback: _('加入备选', 'Add fallback', '加入備選'),
  removeFallback: _('移出备选', 'Remove fallback', '移出備選'),
  ...
  batchDeleteImpactConfig: _('配置会立即保存，不会自动重启 Gateway', 'Configuration is saved immediately without restarting Gateway', '設定會立即儲存，不會自動重啟 Gateway', '設定はすぐに保存されますが、Gateway は自動再起動しません', '설정은 즉시 저장되며 Gateway는 자동으로 재시작되지 않습니다', 'Cấu hình sẽ được lưu ngay mà không tự khởi động lại Gateway', 'La configuración se guardará de inmediato sin reiniciar Gateway', 'A configuração será salva imediatamente sem reiniciar o Gateway', 'Конфигурация будет сохранена немедленно без перезапуска Gateway', 'La configuration sera enregistrée immédiatement sans redémarrer Gateway', 'Die Konfiguration wird sofort gespeichert, ohne Gateway neu zu starten'),
  ...
  applyGateway: _('应用到 Gateway', 'Apply to Gateway', '套用到 Gateway'),
  applyGatewayHint: _('手动重载 Gateway，让已保存的模型配置生效；可能短暂影响正在运行的消息渠道', 'Manually reload Gateway so saved model changes take effect; this may briefly affect active messaging channels', '手動重載 Gateway，讓已儲存的模型設定生效；可能短暫影響正在執行的消息渠道'),
  ...
  routingConsole: _('模型路由控制台', 'Model Routing Console', '模型路由控制台'),
  routePresetTitle: _('一键方案', 'Quick route', '一鍵方案'),
  routeFast: _('快速响应', 'Fast', '快速回應'),
  routeStable: _('稳妥兜底', 'Stable', '穩妥備援'),
  routeContext: _('长上下文', 'Long context', '長上下文'),
  routeReasoning: _('推理优先', 'Reasoning first', '推理優先'),
  routePresetApplied: _('模型路由方案已应用', 'Model route applied', '模型路由方案已套用'),
  activeChainTitle: _('当前生效链 (支持拖拽排序)', 'Active Chain (drag to reorder)', '目前生效鏈 (支援拖曳排序)'),
  activeChainSubtitle: _('主模型失败时按这个顺序依次尝试', 'Tried in this order when the primary model fails', '主模型失敗時會依此順序嘗試'),
  candidatePoolTitle: _('可用候选池 (按服务商分组)', 'Candidate Pool (grouped by provider)', '可用候選池 (按服務商分組)'),
  candidatePoolSubtitle: _('从这里加入备用链，建议跨服务商搭配', 'Add backups from here; cross-provider routes are recommended', '從這裡加入備用鏈，建議跨服務商搭配'),
