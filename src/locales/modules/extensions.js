...
export default {
  ...
  configBtn: _('配置', 'Configure', '設定', '設定', '구성', 'Cấu hình', 'Configurar', 'Configurar', 'Настроить', 'Configurer', 'Konfigurieren'),
  configured: _('已配置', 'Configured', '已設定', '設定済み', '구성됨', 'Đã cấu hình', 'Configurado', 'Configurado', 'Настроено', 'Configuré', 'Konfiguriert'),
  configAvailable: _('可配置', 'Configurable', '可設定'),
  configFor: _('配置 {id}', 'Configure {id}', '設定 {id}', '{id} を設定', '{id} 구성', 'Cấu hình {id}', 'Configurar {id}', 'Configurar {id}', 'Настроить {id}', 'Configurer {id}', '{id} konfigurieren'),
  configKey: _('字段名', 'Field name', '欄位名', 'フィールド名', '필드명', 'Tên trường', 'Campo', 'Campo', 'Поле', 'Nom du champ', 'Feldname'),
  configAddField: _('添加字段', 'Add field', '新增欄位', 'フィールドを追加', '필드 추가', 'Thêm trường', 'Agregar campo', 'Adicionar campo', 'Добавить поле', 'Ajouter un champ', 'Feld hinzufügen'),
  configEmpty: _('暂无配置字段，可添加字段开始配置', 'No config fields yet. Add a field to start.', '暫無設定欄位，可新增欄位開始設定'),
  configKnownFields: _('识别到 {count} 个配置字段', '{count} config fields detected', '識別到 {count} 個設定欄位'),
  configEnum: _('可选值', 'Options', '可選值'),
  configHint: _('配置会保存到 OpenClaw 配置的 plugins.entries.<插件ID>.config；数组和对象请填写合法 JSON。', 'Config is saved to plugins.entries.<pluginId>.config in OpenClaw config. Arrays and objects must be valid JSON.', '設定會儲存到 OpenClaw 設定的 plugins.entries.<插件ID>.config；陣列和物件請填寫合法 JSON。'),
  configSaving: _('保存中...', 'Saving...', '儲存中...', '保存中...', '저장 중...', 'Đang lưu...', 'Guardando...', 'Salvando...', 'Сохранение...', 'Enregistrement...', 'Speichern...'),
  configSaved: _('插件配置已保存', 'Plugin config saved', '插件設定已儲存', 'プラグイン設定を保存しました', '플러그인 구성 저장됨', 'Đã lưu cấu hình plugin', 'Configuración guardada', 'Configuração salva', 'Конфигурация сохранена', 'Configuration enregistrée', 'Plugin-Konfiguration gespeichert'),
  configInvalidNumber: _('不是有效数字', 'Invalid number', '不是有效數字'),
  configInvalidJson: _('不是合法 JSON', 'Invalid JSON', '不是合法 JSON'),
  configExpectedArray: _('需要填写 JSON 数组', 'Expected a JSON array', '需要填寫 JSON 陣列'),
  configExpectedObject: _('需要填写 JSON 对象', 'Expected a JSON object', '需要填寫 JSON 物件'),
  configDuplicateKey: _('字段「{key}」重复', 'Duplicate field "{key}"', '欄位「{key}」重複'),
