...
export default {
  ...
  commandCenter: _('运行控制台', 'Command center'),
  healthScore: _('健康度', 'Health'),
  healthGood: _('系统运行良好', 'System is healthy'),
  healthNeedsAttention: _('有几项需要关注', 'Needs attention'),
  healthCritical: _('核心能力未就绪', 'Core runtime is not ready'),
  healthExternal: _('外部 Gateway 正在占用', 'External Gateway is active'),
  healthSummaryGood: _('Gateway、模型、服务和连接都已就绪，可以直接进入对话或运维。', 'Gateway, models, services, and connection are ready. You can chat or operate from here.'),
  healthSummaryExternal: _('当前 Gateway 不是由此面板启动，建议先确认绑定关系，避免误操作其他实例。', 'The active Gateway was not started by this panel. Confirm the binding before operating.'),
  healthSummaryGatewayDown: _('Gateway 未运行，聊天、渠道和 Control UI 入口会暂时不可用。', 'Gateway is stopped, so chat, channels, and Control UI are temporarily unavailable.'),
  healthSummaryNoModel: _('还没有可用模型，Agent 即使启动也无法稳定响应。', 'No model is configured yet, so agents cannot respond reliably.'),
  healthSummaryPartialServices: _('部分基础服务未运行，建议进入服务页确认是否需要恢复。', 'Some base services are stopped. Review the Services page if they should be restored.'),
  healthSummaryWsPending: _('Gateway 已启动，但 WebSocket 仍未握手完成，实时状态可能延迟。', 'Gateway is running, but WebSocket has not finished handshaking, so live status may lag.'),
  healthSummaryVersion: _('检测到版本提示，建议确认本地版本和推荐稳定版是否一致。', 'A version notice is available. Check whether local and recommended stable versions align.'),
  nextStartGateway: _('启动 Gateway', 'Start Gateway'),
  nextAddModel: _('添加模型', 'Add model'),
  nextCreateAgent: _('创建 Agent', 'Create Agent'),
  nextCheckConnection: _('检查连接', 'Check connection'),
  nextReviewVersion: _('查看版本', 'Review version'),
  nextOpenChat: _('打开聊天', 'Open chat'),
  nextResolveGateway: _('处理外部 Gateway', 'Resolve external Gateway'),
  modelsShort: _('模型', 'models'),
